...
{
    public class SettlementReportPublicEmployeeController : BaseController
    {
      ...
      HumanResource.SettlementReport.Refresh(model);
        ...
        public ActionResult Report(SettlementReportModel model, string savedModel)
        {
            ...
            string path = Path.Combine(Server.MapPath("~/Reports"), "PublicEmployeeReport.rdlc");
            ...
            foreach (var row in model.Grid)
            {
                datasources.Add(new EmployeeReport()
                {
                    ...
                    DirectlydateFrom=row.DirectleyDate.ToString(),
